...
int checkSuffix(ull_int a, ull_int b) { 
  char s1[32], s2[32];
  sprintf(s1, "%lld", a);
  sprintf(s2, "%lld", b);
  int n1 = strlen(s1); 
      ...
      case LS:
        ...
        for(i=0; i<=nDir; i++) {
          if(checkSuffix(nrev(dir[i].addr), x)) {
            printf("\e[0;92m"" %s [dir] \n""\e[0m", dir[i].dname);
          }
        }
        for(i=0; i<=nFil; i++) {
          if(checkSuffix(nrev(file[i].addr), x)) {
            if(strcmp(file[i].fname, "NULL") == 0) continue;
            printf("\e[0;94m"" %s [file] \n""\e[0m", file[i].fname);
          }
        }
        break;
      case CD:
